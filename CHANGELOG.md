...
- Reworked the local HTML report with embedded branding, clearer readiness details, policy status, phase results, and pending-update evidence.
- Replaced the old generic artwork with a legible recovery-ring identity, a real console capture, and a 1280 by 640 social preview.
- Release ZIPs now preserve their asset folders and ship with an external SHA256 manifest beside the JSON receipt.
- Fixed command-line parsing and module forwarding for `-TranscriptPath` so both entry points share the same reporting controls.
- Release tools now load the Windows security and archive modules from their system paths when another module path shadows them.
...
- Added the diagnostic pre-check, selective repair controls, progress tracking, and Application event logging.
- Added the first project icon to the README.

## [v1.0.0] (2026-03-18)

- Published the initial Windows Update repair script.
