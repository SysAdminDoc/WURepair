...
- Platforms and distribution: Windows 10/11 including LTSC/IoT; Windows PowerShell 5.1; GitHub Release ZIP today; planned Authenticode and PowerShell Gallery module delivery.
...
- PSWindowsUpdate: mature PowerShell Gallery module for scan/install/history/settings and `Reset-WUComponents`; learn module packaging, WUA abstractions, and reboot-aware update queries, but avoid becoming a general update installer/blocker.
...
- Verified: no `.psd1`, `.psm1`, `.nuspec`, or `.ps1xml` files exist, so the signed/module packaging roadmap item still needs module metadata and packaging checks before a PowerShell Gallery release.
...
- Distribution/upgrade: Authenticode signing, PowerShell Gallery manifest metadata, release ZIP verification, and optional self-update signature checks are appropriate; GitHub Actions build workflows are not.
...
- https://github.com/mgajda83/PSWindowsUpdate/issues
...
- https://github.com/thatKinji/Reset-WindowsUpdateTools
